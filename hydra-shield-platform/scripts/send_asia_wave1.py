#!/usr/bin/env python3
"""Send the Asia+Brazil investment/consulting wave from its JSON table.
For Vultr where SMTP secrets live.

Safety rules (aligned with the funder/media wave senders):
...
- Idempotent: an entry whose wave email was already logged as sent is
  skipped, so a re-run never double-mails a lead.
"""
...
DATA_PATH = os.path.join(BASE, "marketing", "outreach", "asia_investment_wave1.json")
LEADS_DIR = os.path.join(BASE, "marketing", "leads")
FOLLOWUP_DAYS = 7
WAVE_LABEL = "AsiaWave1"
...
def _lead_context(slug: str, entry: dict) -> dict:
    try:
        lead = json.loads((Path(LEADS_DIR) / f"{slug}.json").read_text(encoding="utf-8"))
    except (OSError, ValueError):
        lead = {}
    return {
        "contact_name": "there",
        "organization": lead.get("organization") or entry.get("org") or "",
        "country": lead.get("country") or entry.get("country") or "",
...
def main() -> int:
    ...
    total = len(entries)
    print("=" * 70)
    print(f"{WAVE_LABEL} | entries: {total} | mode: {'SEND' if not dry else 'DRY RUN'}")
    print(f"caps: wave {DAILY_CAP}/run, platform {PLATFORM_CAP}/day | follow-up: {followup}")
    print("=" * 70)
    for entry in entries:
        ...
        if dry:
            rendered = mailer.render_template(entry["template"], context)
            print("=" * 70)
            print(f"TO: {email} — {entry['org']} ({entry['country']}, {entry['segment']})")
            print("SUBJECT: " + rendered["subject"])
            print(rendered["text"].rstrip())
        else:
            mailer.send_mail(email, entry["template"], context)
            store.add_interaction(
                slug,
                f"{WAVE_LABEL} outreach email sent to {email}",
                type="email",
            )
            sent = sent + 1
