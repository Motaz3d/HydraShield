#!/usr/bin/env python3
"""Build and schedule the European investment/consulting cold-outreach wave.

Segments: investment, environmental_consulting, consultants.
Templates: outreach_investment, outreach_environmental_consulting.
"""
...
OUT_JSON = os.path.join(OUT_DIR, "eu_investment_wave1.json")
OUT_MD = os.path.join(OUT_DIR, "eu_investment_wave1.md")
...
def _iter_target_leads() -> List[Dict]:
    ...
    for path in sorted(glob.glob(os.path.join(LEADS_DIR, "*.json"))):
        ...
        try:
            ...
            continue
        if (lead.get("segment") or "") not in SEGMENT_TEMPLATE:
            continue
...
def _segment_rank(lead: Dict) -> int:
    # investment first, then environmental_consulting, then consultants.
    return {"investment": 0, "environmental_consulting": 1, "consultants": 2}.get(
        lead.get("segment") or "", 99)


def _already_scheduled(store: MarketingStore, slug: str) -> bool:
    for row in store.list_scheduled(lead_slug=slug, status="scheduled"):
        if row.get("template") in SEGMENT_TEMPLATE.values():
            return True
    return False
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=10,
                        help="cap the number of sendable entries (default: 10)")
    parser.add_argument("--today-count", type=int, default=3,
                        help="number of rows to schedule for today (default: 3)")
    ...
    for i, lead in enumerate(selected):
        ...
        if i < today_count:
            send_at = _dt("2026-09-03", 13, 30 + 5 * i)
        else:
            j = i - today_count
            # Tomorrow after banking wave (which ends at 08:05 + 7 rows = 08:35).
            send_at = _dt("2026-09-04", 8, 40 + 5 * j)
    ...
    lines: List[str] = []
    lines.append("# Talaix — European Investment/Consulting Outreach Wave 1")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Segments: investment, environmental_consulting, consultants.")
    ...
    lines.append("- Templates: `outreach_investment`, `outreach_environmental_consulting`.")
    lines.append("")
    ...
    for i, s in enumerate(schedule, 1):
        lines.append(f"## {i}. {s['organization']} — {COUNTRY_NAMES.get(s['canonical_country'], s['country'])} ({s['segment']})")
        lines.append("")
    ...
    if inferred_only:
        ...
        for r in inferred_only:
            lines.append(f"- {r['org']} ({r['country']}, {r['segment']}, `{r['slug']}`): {r['inferred_email']}")
        lines.append("")
    ...
    if contact_gap:
        ...
        for r in contact_gap:
            lines.append(f"- {r['org']} ({r['country']}, {r['segment']}, `{r['slug']}`)")
        lines.append("")
    ...
    print(f"Scheduled: {len(schedule)} EU investment/consulting leads")
