...
- `src/` — main Python package (importable as `src`):
  ...
  - Report language (binding convention, operator directive 2026-09-14): customer-facing documents print business labels, never internal identifiers (`covered_by_evidence`, `hazard_match_only`, `fire_weather`…). Machine values stay the API/SDK contract. Label helpers: `coverage_label()` (`src/climate/sustainability.py`), `fit_band_label()` (`src/climate/solutions.py`), `component_label()` (`src/dashboard/ignition.py`), `human_label()` (`src/dashboard/report_language.py`). Table cells must be `Paragraph` (a raw string prints pre-escaped markup literally — `&amp;` — and never wraps; use `_as_cell` in `src/dashboard/verification_report.py`). Contract: `tests/test_report_language.py` scans the shipped PDFs/pages for `snake_case`.
...
- `website/` — static site (HTML/CSS/JS). Clarity-first IA (2026-09-14, supersedes the 2026-09-13 challenge-first hero — external review scored product clarity 6.5/10, new-visitor message clarity 6/10): homepage = positioning headline ("Climate-risk evidence your auditor can trace") + 3-step "how it works" + 3 challenge cards + sample pack + 6 audience cards + live monitor; the full product map moved to `capabilities.html`; `sample.html` + `assets/samples/talaix-sample-evidence-pack.pdf` = the deliverable, readable without an account (regenerate with `scripts/build_sample_pack.py`, `--check` verifies); `about.html` = who is behind the evidence. Nav in `js/chrome.js` = How it works · Who it's for · Capabilities ▾ (7 core doors: one-page map · compliance overview · CSRD · Green Finance DNSH · EUDR · Insurance · Maps) · Pricing; specialist tools (forensics, licensing, siting/funding, reports, academy, verify, sources) are demoted out of the dropdown via `nav:false` but stay reachable from the footer + `capabilities.html` (nothing lost). Contract locked by `tests/test_site_clarity.py`. Purchase path (2026-09-14): `js/buy.js` renders the one reusable "buy it" block — live prices from `/api/v2/billing/config`, what the buyer receives, the three steps, an invoice-by-email route, and a guest path that keeps the choice + location across the sign-in — auto-loaded from chrome.js, mounted via `[data-buy-report]`/`[data-buy-tier]` on green-finance, insurance, sample and sustainability; `js/postpurchase.js` (account.html) reacts to Stripe's return flags (`purchased=report`, `subscribed=1`, `checkout=cancelled`) and resumes an interrupted purchase. Contract: `tests/test_buy_path.py`. Production billing is LIVE (`billing_enabled: true`, live publishable key); report and tier checkout both require a signed-in account. Guided stepper: `js/quickpath.js` (auto-loaded from chrome.js, no-op without a mount) renders a progressive-disclosure tunnel via `<div class="quickpath" data-mode="hide" data-steps="id:Label,...">` — CSRD, Green Finance, EUDR, Insurance, Forensics, Licensing. Search-assist: `js/search-assist.js` gives every form field (text/number/textarea) a context helper dropdown.
