...
| Compliance outreach wave 1 — script built, preview pending on Vultr | 🔶 | Operator approved 2026-09-06. `compliance_wave1.md` + `compliance_wave1.json` (8 verified Tier-1 targets; Position Green = webform-only, operator submits manually) + template `outreach_sustainability_compliance` + `scripts/send_compliance_wave1.py` (dry-run default, stagger/webform gates, caps, idempotent) + `scripts/send_preview_compliance_wave.py` (preview → info@talaix.com; local run lands in `data/outbox/`). 5 new tests green. Next: run preview on Vultr, then `--send` in staggered batches |
