...
| Compliance-first identity pivot on site | ✅ | Hero/title/meta/JSON-LD → "Climate-risk compliance evidence for EU disclosure"; nav reordered (Compliance first); footer brand line; llms.txt rewritten; `compliance.html` gained the who-must-comply wave calendar |
...
| Compliance outreach wave 1 (draft, awaiting operator review) | 🔶 | `marketing/outreach/compliance_wave1.md`: 3 tiers — 8 suite-vendor partner targets with official sourced contacts (Workiva, osapiens, Position Green (form only), Sweep, Atlas Metrics, Clarity AI, Greenly, Watershed; verified on official sites 2026-09-06), consultants sourcing protocol, direct companies deferred to real leads discovery. Human-gated, nothing sent; build script (build_eu_* pattern) comes after review |
